  ...
  const renderQA = (qa) => {
    return qa.map((item, index) => {
      return <div key={index}>
        ...
        <p className=' transition-colors delay-200 text-left sm:text-xl text-l font-bold font-mono text-accent-2 dark:text-secondary p-3 pb-0 '>{item.question}</p>
        <p className='transition-colors delay-200 text-left sm:text-l text-base font-bold font-mono text-accent-1 dark:text-secondary-shade p-3 py-0 pb-3 '>{item.answer}</p>
        </div>
      </div>
    })
  }
    ...
    <div className='animate-appear sm:m-6 m-3 transition-all delay-200 bg-secondary-shade dark:bg-accent-shade rounded-3xl'>
        ...
        <div className='transition-colors delay-200 bg-transparent rounded-b-3xl p-3'>
          ...
          <form onSubmit={handleSubmit} className="flex " >
            ...
            <input disabled={isLoading} value={input} onChange={handleChange} placeholder='Ask anything related to the headline...'
            ...
            {isLoading ? <FontAwesomeIcon className='p-3 text-accent-2 dark:text-primary animate-spin sm:text-xl text-l' icon={faCircleNotch }/> : null}
          </form> 
        </div>
    </div>
  )
